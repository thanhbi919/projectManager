...
const userPermissions = ref(localStorage.getItem('department') || '')
console.log(userPermissions)

// Các nhóm menu ban đầu
    ...
    menuItems: [
      {
        ...
        label: 'Dashboard',
        route: '/',
        permissions: null // Không yêu cầu quyền cụ thể
      },
      {
        icon: `<svg
                  class="fill-current"
        ...
        label: 'User',
        route: '/users',
        permissions: ['admin'] // Yêu cầu quyền admin
      },
      {
        icon: `<svg
                  class="fill-current"
        ...
        label: 'Projects',
        route: '/projects',
        permissions: null // Yêu cầu quyền admin hoặc pm
      },
      {
        icon: `<svg
                  class="fill-current"
...
// Lọc các mục menu dựa trên quyền
const filteredMenuGroups = computed(() => {
  return menuGroups.value.map((group) => ({
    ...group,
    menuItems: group.menuItems.filter((item) => {
      // Nếu không có permissions yêu cầu, luôn hiển thị
      if (!item.permissions) return true
      // Nếu có, kiểm tra quyền người dùng
      return item.permissions.includes(userPermissions.value)
    })
  }))
})
</script>
